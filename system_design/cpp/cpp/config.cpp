...
shared_ptr<CeleritasConfig> initCeleritasConfig(pyobj python_config) {
    shared_ptr<CeleritasConfig> ret_config = std::make_shared<CeleritasConfig>();
...
shared_ptr<CeleritasConfig> initConfig(string config_path) {

    shared_ptr<CeleritasConfig> ret;
    if (Py_IsInitialized() != 0) {
        string module_name = "celeritas.utils.configs.celeritas_model_configs";
        ...
        ret = initCeleritasConfig(python_config);
    } else {
